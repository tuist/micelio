...
| `micelio_factory_operation_duration_seconds{operation,outcome}` | Are durable graph-run or account inference-profile operations slow or failing? Operation and outcome are bounded, so repository work does not create metric-label cardinality. |
| `micelio_factory_operation_count{operation,outcome}` | Which durable graph-run or account inference-profile operations are succeeding or failing? |
